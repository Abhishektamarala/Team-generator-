...
let team_generator = function(team_one,team_two,team_one_index,team_two_index,mn,csv,cev,fixed_one,fixed_two,captain_one,captain_two,vice_captain_one,vice_captain_two)
{
   ...
   teams_data = JSON.parse(localStorage.getItem('teams_data'))
   ...
   custom_strategy(wk_cnt,bat_cnt,al_cnt,bowl_cnt,selected_team_one,selected_team_two,team_one_index,team_two_index,mn,csv,cev,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players)
}
let custom_strategy = function(wk_cnt,bat_cnt,al_cnt,bowl_cnt,selected_team_one,selected_team_two,team_one_index,team_two_index,mn,csv,cev,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players)
{
   c_strategy=[]
   f_wk=0
   f_bat=0
   ...
   sc.addEventListener('click',()=>
   {
      ...
      team_generator_helper_one(selected_team_one,selected_team_two,team_one_index,team_two_index,nt,mn,csv,cev,selected_strategies,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl)
      })

   })

}
let team_generator_helper_one = function(st_one,st_two,toi,tti,nt,mn,csv,cev,strategy,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl)
{
   wk=[]
   bat=[]
   al=[]
  ...
  // console.log(strategy)
   team_generator_helper_two(wk,bat,al,bowl,fp_wk,fp_bat,fp_al,fp_bowl,strategy,toi,tti,nt,mn,csv,cev,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl)
}
...
let get_hash_value = function(team,captain_index,vice_captain_index)
{
   team.sort((x,y)=>
   {
      if(x.player_image<y.player_image) return -1
      else return 1
   })
   let hash=0
   team.forEach((player,index)=>
   {
      let temp = (player.player_image)*Math.pow(3,index+1)
      if(player.player_image==captain_index)
         temp=temp*5
      if(player.player_image==vice_captain_index)
         temp=temp*3
      hash+=temp
   })
   return hash
}
let team_generator_helper_two = function(wk,bat,al,bowl,fp_wk,fp_bat,fp_al,fp_bowl,strategy,toi,tti,nt,mn,csv,cev,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl)
{
   let cnt =0
   ...
   while(cnt<nt)
   {
      ...
      if(validation_one(team,toi,tti) && credits >=csv && credits <=cev)
      {
         c_p=[]
         vc_p=[]
         c1_players.forEach((player)=>{c_p.push(player.player_image)})
         c2_players.forEach((player)=>{c_p.push(player.player_image)})
         vc1_players.forEach((player)=>{vc_p.push(player.player_image)})
         vc2_players.forEach((player)=>{vc_p.push(player.player_image)})
         s_vp=[]
         team.forEach((player)=>{s_vp.push(player.player_image)})
         final_c=[]
         final_vc=[]
         s_vp.forEach((data)=>{
        ...
        // console.log(hashmap)
         ...
         if(flag==-1)
         {
            team_obj = create_team(team,toi,tti,captain_id,vice_captain_id,cnt+1,credits)
  ...
  // console.log(map_cnt)
   ...
   let attempt_id = Number(localStorage.getItem('start_id'))+1
   localStorage.setItem('start_id',`${attempt_id}`)
   let attempt = new Attempt(team_list,nt,attempt_id,toi,tti)
   let req_index = create_or_update_match(attempt,mn,toi,tti)
   gp = document.getElementById('generate_panel')
   gp.style.display="none"
   display_teams(mn,req_index)
}

let create_or_update_match = function(attempt,mn,toi,tti)
{
   let flag = localStorage.getItem(`${mn}`)
   if(flag==null)
   {
      match =new Match(mn,toi,tti)
      match.attempts.push(attempt)
      localStorage.setItem(`${mn}`,JSON.stringify(match))
      return 0
   }
   else{
      match_obj = JSON.parse(flag)
      match_obj.attempts.push(attempt)
      localStorage.setItem(`${mn}`,JSON.stringify(match_obj))
