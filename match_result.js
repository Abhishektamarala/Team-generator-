...
mr.addEventListener('click',()=>
{
    ...
    get_match_result_number()
})
let get_match_result_number = function()
{
    ...
    mrnc.addEventListener('click',()=>
    {
        ...
        if(mrnv!=null)
        {
            if(localStorage.getItem(`${mrnv}`)==null){raiseError('Match Number does not exist! kindly generate Match Number'); return}
            console.log('everything is fine')
            fetch_match(Number(mrnv));
        }
        else
        {
...
let fetch_match = function(mn)
{
    ...
    match_obj = JSON.parse(localStorage.getItem(`${mn}`))
    ...
    if(cr!=null)
    {
        cr.forEach((obj,index)=>
        {
            obj.addEventListener('click',()=>
            {
                ...
                calc_first_ten(mn,index)
            })
        })
    }
    result_helper(mn)
}
let calc_first_ten = function(mn,attempt_index)
{
   // console.log(attempt_id)
    let temp = localStorage.getItem(`${mn}`)
    rv= document.getElementById('results_vp')
    rv.style.display="block"
    obj = JSON.parse(temp)
    result_teams = get_result_teams(obj,attempt_index)
    
}
let get_result_teams = function(obj,attempt_index)
{
    attempt =null
    ...
    {
        result_list.push(new Item(index,get_team_score(obj,team,attempt)))
    })
    ...
    for(let i=0;i<10;i++)
    {
        ...
        vp_team = get_selected_vp_team(temp_team)
        wk=[]
        bat=[]
        al=[]
        bowl=[]
        ...
        team_result_placer.appendChild(final_team_creation(wk,bat,al,bowl,attempt.team_list[result_list[i].index].captain,attempt.team_list[result_list[i].index].vice_captain,attempt.team_list[result_list[i].index].team_number,result_list[i].score,1))
    }
}
let get_selected_vp_team = function(team)
{
    let teams_data = localStorage.getItem('teams_data')
    teams_data_obj = JSON.parse(teams_data)
...
let get_team_score = function(obj,team,attempt)
{
    let teams_data = localStorage.getItem('teams_data')
    teams_data_obj = JSON.parse(teams_data)
    team1 = teams_data_obj.teams[team.team_one_index].players
    ...
    selected_players_one.forEach((player,index)=>
    {
        ...
        if(player.player_image==team.captain)
            sum+=temp*2
        else if(player.player_image==team.vice_captain)
            sum+=temp*1.5
        else
            sum+=temp
    })
...
let result_helper = function(mn)
{
    ...
    {
        get_player_points(mn)
    })
    // logic for #check_teams
}
let get_player_points = function(mn)
{
    let team_list=['KKR','MI','SRH','RCB','KXIP','DC','RR','CSK']
    md = document.getElementById('match_detail')
    md.style.display="none"
